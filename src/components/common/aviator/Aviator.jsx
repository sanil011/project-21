...
const Aviatior = () => {
    ...
    // 🟠 If Game is Running -> Process Cashout
      ...
      try {
        ...
        if (data.data.ifEncashed === true) {
          setWinMessage(`You Won: ₹${data.data.encashedAmount}`);
  ...
  const handleAutoCashoutFirst = () => {
    ...
    setWinMessage(`You Won: ₹${(autoCashOutFirst * firstBetAmount).toFixed(2)}`);
  ...
  const handleAutoCashoutSecond = () => {
    // ✅ Show "WON" message when auto cashout triggers
    setWinMessage(`You Won: ₹${(autoCashOutSecond * secondBetAmount).toFixed(2)}`);
  ...
  return (
    <div className="bg-[#111013] min-h-screen -mb-4">
      ...
      <Box className="h-fit mx-2 rounded-lg overflow-hidden flex justify-center items-center text-white font-semibold text-2xl">
        ...
        {winMessage && (<div className="win-message show">{winMessage}</div>)}
      </Box>
        ...
        <Box className="bg-[#1f1f1f] rounded-xl  w-full">
          ...
          <div className="p-2">
            ...
            <Box className="flex gap-2 w-full">
              <Box className='w-[35%]'>
                <Box className="flex gap-2 items-center bg-[#111013] px-2 py-0.5 rounded-full border">
                  <div
                    className="flex items-center justify-center text-xs border border-gray-400 rounded-full w-4 h-4 cursor-pointer"
                    onClick={() => handleBetAmountChange("first", "subtract")}
                  >
                    <SubtractIcon />
                  </div>
                  <input
                    ...
                    className="text-white text-center w-14 sm:w-16 p-0 focus:outline-none focus:ring-0"
                  />
                  <div
                    className="flex items-center justify-center text-xs border border-gray-400 rounded-full w-4 h-4 cursor-pointer"
                    onClick={() => handleBetAmountChange("first", "add")}
                  >
                    <PlusIcon />
                  </div>
                </Box>
            ...
            <Box className="flex gap-2 w-full">
              <Box className='w-[35%]'>
                ...
                <Box className="flex gap-2 items-center bg-[#111013] px-2 py-0.5 rounded-full border">
                  <div
                    className="flex items-center justify-center text-xs border border-gray-400 rounded-full w-4 h-4 cursor-pointer"
                    onClick={() => handleBetAmountChange("second", "subtract")}
                  >
                    <SubtractIcon />
                  </div>
                  <input
                  ...
                  <div
                    className="flex items-center justify-center text-xs border border-gray-400 rounded-full w-4 h-4 cursor-pointer"
                    onClick={() => handleBetAmountChange("second", "add")}
                  >
                ...
                <Box className="grid grid-cols-2 gap-0.5 mt-0.5">
                  {betOptions.map((bet, index) => (
                    <div
                      key={index}
                      className="text-center text-xs py-0.5 bg-[#111013] text-gray-500 rounded-full cursor-pointer"
                      onClick={() => setSecondBetAmount(bet)}
                    >
                      {bet}.00
                    </div>
                  ))}
